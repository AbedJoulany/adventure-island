...
public:
    ...
    MovingObject(const float width, const float height, const std::shared_ptr<Board>& board);
    ...
    virtual void jump();   
    virtual void updatePosition(const float gravity);  
...
private:
    ...
    float bufferedRoundoff(float i) const;
