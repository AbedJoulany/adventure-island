...
Enemy::Enemy(const sf::Vector2f position, const std::shared_ptr<Board>& board , const std::string id, const std::string path,
    int first, int last, bool moving) :
    MovingObject(OBJECTWIDTH, OBJECTHEIGHT, board),
    m_first(first),
    m_last(last),
    m_isMoving(moving),
    m_killed(ENEMYKILLED),
...
void Enemy::updatePosition(const float gravity)
{
    if (m_isMoving)
    {
        /* UPDATE LAST TEXTURE */
